...
username_to_id = {}
bot = telebot.TeleBot('5663670515:AAGUZechpmYyj_JjTdsBBAmdiTNfUjO_UPA')
...
def get_statistic(message):
    print('in handler statistic')
    bot.send_message(message.chat.id, 'Number of members: ' + str(bot.get_chat_members_count(message.chat.id)) +
                     '\nNumber of admins: ' + str(len(bot.get_chat_administrators(message.chat.id))))
@bot.message_handler(commands=['leave'])
def leave(message):
    print('in handler leave')
    bot.send_message(message.chat.id, 'Bye')
    bot.leave_chat(message.chat.id)

@bot.message_handler(content_types = ['new_chat_members'])
def question_message(message):
    print('in handler new chat')
    ...
    bot.send_message(message.chat.id,"Какой ты капибобрик сегодня?", reply_markup=markup)

@bot.callback_query_handler(func = lambda _: True)
def get_callback(callback):
    print('in handler callback')
    if(callback.data == 'sad_capy'):
        bot.send_message(callback.message.chat.id,'Это ты!')
        bot.send_photo(callback.message.chat.id, sad_capy)
    elif (callback.data == 'happy_capy'):
        bot.send_message(callback.message.chat.id,'Это ты!')
        bot.send_photo(callback.message.chat.id, happy_capy)

@bot.message_handler(commands = ['promote'],func= lambda message: bot.get_chat_member(message.chat.id, message.from_user.id).status in ['administrator','creator'])
def promote(message):
    print('in handler promote')
    if(len(message.text.split()) > 1 and len(message.text.split()[1]) > 1):
        username = message.text.split()[1][1:]
        if(username in username_to_id.keys()):
            bot.promote_chat_member(message.chat.id,username_to_id[username],True,True,True,True,True,True,True,True)
        else:
            bot.send_message(message.chat.id,'I don\'t know this capy... Let this capy write smth to the chat in capylang')
    else:
        bot.send_message(message.chat.id,'Bad request(')

@bot.message_handler(commands = ['ban', 'unban'],func= lambda message: bot.get_chat_member(message.chat.id, message.from_user.id).status in ['administrator','creator'])
def unban(message):
    print('in handler unban')
    if(len(message.text.split()) > 1 and len(message.text.split()[1]) > 1):
        username = message.text.split()[1][1:]
        if(username in username_to_id.keys()):
            if('unban' in message.text.split()[0]):
                bot.unban_chat_member(message.chat.id,username_to_id[username])
            else:
                bot.ban_chat_member(message.chat.id,username_to_id[username])
    else:
        bot.send_message(message.chat.id,'Bad request(')
...
def update_dict (message):
    if(len([word for word in message.text.split() if word.lower() in abusive_language])>0):
        ...
        bot.send_photo(message.chat.id,angry_capy)
